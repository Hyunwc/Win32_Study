...
void iOpenAL::initBuffer(int index, const char* szFormat, ...)
{
	...
	char* data = &buf[sizeof(WaveHeader0) + (head0->fmtSize - 16)
			+ sizeof(WaveHeader1)];
...
void iOpenAL::initSource(int idx, bool repeat)
{
	...
	// x, y, distance
	const ALfloat sourcePosition[] = { 0.0f, 0.0f, 0.0f };
	alSourcefv(gSource[idx], AL_POSITION, sourcePosition);

	alSourcef(gSource[idx], AL_REFERENCE_DISTANCE, 50);
